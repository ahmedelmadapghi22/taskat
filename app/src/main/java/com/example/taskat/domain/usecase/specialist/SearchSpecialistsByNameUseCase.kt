...
class SearchSpecialistsByNameUseCase @Inject constructor(private val iSpecialistRepository: ISpecialistRepository) {


    suspend operator fun invoke(searchQuery: String): Flow<UseCaseResult<List<Specialist>, String>> {
        return flow {
            try {
                val resultFlow = iSpecialistRepository.searchSpecialistByName(searchQuery)
