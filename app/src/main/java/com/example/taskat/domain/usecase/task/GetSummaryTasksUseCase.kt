...
class GetSummaryTasksUseCase @Inject constructor(private val iTaskRepository: ITaskRepository,
                                                 private val getCurrentDateUseCase: GetCurrentDateUseCase) {

    suspend operator fun invoke(): UseCaseResult<SummaryTask, String> {
        return try {
            val currentDate = getCurrentDateUseCase().run {
                "$day-$month-$year"
            }
            val result = iTaskRepository.getSummaryToday(currentDate)
