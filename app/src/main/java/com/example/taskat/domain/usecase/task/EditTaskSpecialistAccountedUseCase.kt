...
class EditTaskSpecialistAccountedUseCase @Inject constructor(private val iTaskRepository: ITaskRepository) {
    suspend operator fun invoke(taskID: Int): UseCaseResult<Boolean, String> {
        try {
            if (taskID != -1) {
                val result = iTaskRepository.editIsSpecialistAccounted(taskID)
                if (result > 0)
