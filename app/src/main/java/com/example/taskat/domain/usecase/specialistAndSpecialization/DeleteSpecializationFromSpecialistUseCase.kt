...
class DeleteSpecializationFromSpecialistUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        try {
            if (specialistID != 0 && specializationID != 0) {
                val res = iSpecialistAndSpecializationRepository.deleteSpecializationFromSpecialist(
                    specialistID, specializationID
                )
                if (res > 0) {
